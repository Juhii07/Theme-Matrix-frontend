...
const AdminForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      await axios.post("http://localhost:5000/adminapi/forgot-password", { email })
      setSuccess(true)
    } catch (err) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#faf5ff", display: "flex", alignItems: "center", justifyContent: "center", padding: "16px", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
      ...
      <div style={{ background: "#fff", borderRadius: "20px", padding: "40px 36px", width: "100%", maxWidth: "420px", boxShadow: "0 8px 40px rgba(109,40,217,0.10)" }}>
        ...
        <h2 style={{ textAlign: "center", fontWeight: "900", fontSize: "22px", marginBottom: "6px", color: "#111" }}>Forgot Password?</h2>
        <p style={{ textAlign: "center", color: "#9ca3af", fontSize: "14px", marginBottom: "28px" }}>Enter your admin email to get a reset link</p>
        ...
        {!success ? (
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: "18px" }}>
              ...
              <input
                ...
                style={{ width: "100%", padding: "12px 16px", borderRadius: "12px", border: error ? "2px solid #ef4444" : "2px solid #e5e7eb", fontSize: "14px", outline: "none", boxSizing: "border-box", fontFamily: "inherit", transition: "border-color 0.2s" }}
                onFocus={e => { if (!error) e.target.style.borderColor = "#a855f7" }}
                onBlur={e  => { if (!error) e.target.style.borderColor = "#e5e7eb" }}
              />
              {error && <p style={{ color: "#ef4444", fontSize: "12px", fontWeight: "600", marginTop: "6px" }}>{error}</p>}
            </div>

            <button type="submit" disabled={loading}
              style={{ width: "100%", padding: "13px", borderRadius: "12px", background: loading ? "#c4b5fd" : "linear-gradient(135deg,#a855f7,#7c3aed)", color: "#fff", fontWeight: "900", fontSize: "15px", border: "none", cursor: loading ? "not-allowed" : "pointer", marginBottom: "20px" }}>
              {loading ? (
          ...
          <div>
            <div style={{ background: "#f0fdf4", border: "1.5px solid #bbf7d0", borderRadius: "12px", padding: "20px 16px", marginBottom: "20px", textAlign: "center" }}>
              <div style={{ fontSize: "36px", marginBottom: "10px" }}>✅</div>
              <p style={{ color: "#15803d", fontWeight: "700", fontSize: "15px", marginBottom: "6px" }}>Reset link sent!</p>
            ...
            <button onClick={() => { setSuccess(false); setEmail("") }}
              style={{ width: "100%", padding: "12px", borderRadius: "12px", background: "#f3f4f6", color: "#6b7280", fontWeight: "700", fontSize: "14px", border: "none", cursor: "pointer" }}>
              Try another email
            </button>
          </div>
        )}
