...
const VendorEditProfile = () => {
  ...
  const getVendorData = async () => {
    try {
      const res = await axios.get("http://localhost:5000/vendorapi/me", { headers: H })
      const vendor = res.data.vendor || res.data
      setName(vendor.name   || "")
      setEmail(vendor.email  || "")
      setMobile(vendor.mobile || "")
    } catch (e) {
      setName(localStorage.getItem("vendorName") || "")
    }
  }

  useEffect(() => { getVendorData() }, [])
  ...
  const updateVendor = async () => {
    ...
    try {
      const res = await axios.put(
        `http://localhost:5000/vendorapi/update/${vendorId}`,
        { name, email, mobile },
        { headers: H }
      )
  ...
  const changePassword = async () => {
    ...
    try {
      await axios.post(
        `http://localhost:5000/vendorapi/change-password/${vendorId}`,
  ...
  return (
    <div>
      <div className="mb-7">
        ...
        <p className="text-sm text-gray-400 mt-0.5">Update your vendor account details</p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl overflow-hidden border border-violet-100"
            ...
            <div className="flex flex-col items-center py-8 px-6"
              ...
              <h3 className="font-black text-gray-900 text-lg">{name || "Vendor"}</h3>
            ...
            <div className="px-6 pb-6">
              <button
                ...
                style={{ background: "linear-gradient(135deg,#a855f7,#7c3aed)", border: "none", cursor: "pointer" }}
                onClick={() => document.getElementById("vendorEditForm").scrollIntoView({ behavior: "smooth" })}>
                ✏️ Edit Profile
              </button>
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div id="vendorEditForm" className="bg-white rounded-2xl border border-violet-100"
            style={{ boxShadow: "0 2px 16px rgba(109,40,217,0.06)" }}>
            <div className="px-6 py-4 border-b border-violet-50">
              <h3 className="font-black text-gray-900">👤 Vendor Information</h3>
              <p className="text-xs text-gray-400 mt-0.5">Your registered vendor details</p>
            </div>
            <div className="p-6 space-y-4">
              ...
              <button onClick={updateVendor} disabled={loading}
